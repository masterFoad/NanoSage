...
load_dotenv(os.path.expanduser("~/.env"))  # Proper home directory expansion
load_dotenv()  # Load local .env

# Check API key early if using non-local model
if os.getenv("NANOSAGE_API_KEY") is None:
    print("[WARN] NANOSAGE_API_KEY not found in environment variables or .env file")
    print("[WARN] This will cause errors if using external API models")


def call_gemma(prompt, model=None, personality=None):
    model = model or "gemma2:2b"  # Default if not specified
    system_message = ""
    if personality:
...
def chain_of_thought_query_enhancement(query, personality=None, model=None):
    ...
    raw_output = call_gemma(prompt, personality=personality, model=model)
    return extract_final_query(raw_output)


def summarize_text(text, max_chars=24000, personality=None, model=None):
    model = model or "gemma2:2b"
    if len(text) <= max_chars:
        prompt = f"Please summarize the following text succinctly:\n\n{text}"
        return call_gemma(prompt, personality=personality, model=model)
    # If text is longer than max_chars, chunk it
    chunks = [text[i : i + max_chars] for i in range(0, len(text), max_chars)]
    summaries = []
    for i, chunk in enumerate(chunks):
        prompt = f"Summarize part {i+1}/{len(chunks)}:\n\n{chunk}"
        summary = call_gemma(prompt, personality=personality, model=model)
        summaries.append(summary)
        time.sleep(1)
    combined = "\n".join(summaries)
    if len(combined) > max_chars:
        prompt = f"Combine these summaries into one concise summary:\n\n{combined}"
        combined = call_gemma(prompt, personality=personality, model=model)
    return combined


def rag_final_answer(
    aggregation_prompt, rag_model="gemma", personality=None, base_url=None
):
    print(
        f"[INFO] Performing final RAG generation using model: {rag_model} ({base_url})"
    )
    # save the prompt to a file
    with open("aggregation_prompt.txt", "w") as f:
        f.write(aggregation_prompt)
    if rag_model == "gemma":
        ...
        return call_openai(aggregation_prompt, model=rag_model, base_url=base_url)


def follow_up_conversation(follow_up_prompt, personality=None, model="gemma2:2b"):
    return call_gemma(follow_up_prompt, personality=personality, model=model)
...
def check_api_health(base_url: str, model: str):
    """Perform a basic API health check"""
    try:
        nanosage_api_key = os.getenv("NANOSAGE_API_KEY")
        if not nanosage_api_key:
            print("NANOSAGE_API_KEY not found in environment variables")
            sys.exit(1)
        else:
            print("NANOSAGE_API_KEY found in environment variables")
        client = OpenAI(base_url=base_url, api_key=nanosage_api_key)
        # Test with a tiny prompt
        test_prompt = "Respond only with 'pong'"
        ...
        if response.choices[0].message.content.strip().lower() != "pong":
            raise ValueError("Unexpected API response")
        print(f"[INFO] API health check passed for {base_url}")
    except Exception as e:
        print(f"[FATAL] API health check failed: {e}")
        print("Verify your NANOSAGE_API_KEY and base URL")
        sys.exit(1)
...
class SearchSession:
    def __init__(
        ...
        self.ddg_proxy = ddg_proxy
        self.ollama_model = ollama_model
        self.max_context = max_context

        self.query_id = str(uuid.uuid4())[:8]
        self.base_result_dir = os.path.join(
            self.config.get("results_base_dir", "results"), self.query_id
        )
        os.makedirs(self.base_result_dir, exist_ok=True)

        print(f"[INFO] Initializing SearchSession for query_id={self.query_id}")

        # Strict API key check for external services
        if self.rag_model not in ["gemma", "pali"] and not os.getenv(
            "NANOSAGE_API_KEY"
        ):
            print(
                "[FATAL] NANOSAGE_API_KEY required for external models. "
                f"Current model: {self.rag_model} using base URL: {self.base_url}"
            )
            sys.exit(1)

        # After API key check
        if self.rag_model not in ["gemma", "pali"]:
            print(f"[INFO] Performing API health check to {self.base_url}...")
            check_api_health(self.base_url, self.rag_model)

        # Enhance the query via chain-of-thought.
        self.enhanced_query = chain_of_thought_query_enhancement(
            self.query, personality=self.personality, model=self.ollama_model
        )
        ...
        self.model, self.processor, self.model_type = load_retrieval_model(
            model_choice=self.retrieval_model, device=self.device
        )
        ...
        self.kb = KnowledgeBase(
            self.model, self.processor, model_type=self.model_type, device=self.device
        )
    ...
    async def run_session(self):
        ...
        """
        print(
            f"[INFO] Starting session with query_id={self.query_id}, max_depth={self.max_depth}"
        )
        plain_enhanced_query = clean_search_query(self.enhanced_query)

        # 1) Generate subqueries from the enhanced query
        initial_subqueries = split_query(
            plain_enhanced_query, max_len=self.config.get("max_query_length", 200)
        )
        print(
            f"[INFO] Generated {len(initial_subqueries)} initial subqueries from the enhanced query."
        )
        ...
        if self.config.get("monte_carlo_search", True):
            ...
            initial_subqueries = self.perform_monte_carlo_subqueries(
                plain_enhanced_query, initial_subqueries
            )

        # 3) If web search is enabled and max_depth >= 1, do the recursive expansion
        if self.web_search_enabled and self.max_depth >= 1:
            web_results, web_entries, grouped, toc_nodes = (
                await self.perform_recursive_web_searches(
                    initial_subqueries, current_depth=1
                )
            )
            ...
            print(
                "[INFO] Web search is disabled or max_depth < 1, skipping web expansion."
            )
    ...
    def perform_monte_carlo_subqueries(self, parent_query, subqueries):
        ...
        print(
            f"[DEBUG] Monte Carlo: randomly picking up to {max_subqs} subqueries from {len(subqueries)} total."
        )
        scored_subqs = []
        for sq in subqueries:
            ...
            if not sq_clean:
                continue
            node_emb = embed_text(
                sq_clean, self.model, self.processor, self.model_type, self.device
            )
        ...
        if not scored_subqs:
            print(
                "[WARN] No valid subqueries found for Monte Carlo. Returning original list."
            )
            return subqueries
    ...
    async def perform_recursive_web_searches(self, subqueries, current_depth=1):
        ...
        for sq in subqueries:
            ...
            relevance = late_interaction_score(
                self.enhanced_query_embedding, node_embedding
            )
            toc_node.relevance_score = relevance

            if relevance < min_relevance:
                print(
                    f"[INFO] Skipping branch '{sq_clean}' due to low relevance ({relevance:.2f} < {min_relevance})."
                )
                continue
            ...
            print(
                f"[DEBUG] Searching web for subquery '{sq_clean}' at depth={current_depth}..."
            )

            # Get retry parameters from config
            ddg_retries = self.config.get("ddg_search_retries", 3)
            ddg_timeout = self.config.get("ddg_search_timeout", 15)
            ddg_retry_delay = self.config.get("ddg_retry_delay", 3)

            pages = await download_webpages_ddg(
                sq_clean,
                limit=self.config.get("web_search_limit", 5),
                output_dir=subquery_dir,
                proxy=self.ddg_proxy,
                ddg_timeout=ddg_timeout,
                ddg_retries=ddg_retries,
                ddg_retry_delay=ddg_retry_delay,
            )
            branch_web_results = []
            ...
            # Summarize
            branch_snippets = " ".join(
                [r.get("snippet", "") for r in branch_web_results]
            )
            toc_node.summary = summarize_text(
                branch_snippets,
                max_chars=self.max_context,
            ...
            if current_depth < self.max_depth:
                additional_query = chain_of_thought_query_enhancement(
                    sq_clean, personality=self.personality, model=self.ollama_model
                )
                if additional_query and additional_query != sq_clean:
                    additional_subqueries = split_query(
                        additional_query,
                        max_len=self.config.get("max_query_length", 200),
                    )

            if additional_subqueries:
                deeper_web_results, deeper_corpus_entries, _, deeper_toc_nodes = (
                    await self.perform_recursive_web_searches(
                        additional_subqueries, current_depth=current_depth + 1
                    )
                )
    ...
    def _build_final_answer(
        ...
        toc_str = (
            build_toc_string(self.toc_tree) if self.toc_tree else "No TOC available."
        )
...
"""
    ...
    def save_report(self, final_answer, previous_results=None, follow_up_convo=None):
        ...
        follow_up_prompt = follow_up_convo or ""
        follow_up_convo = follow_up_conversation(
            follow_up_prompt, personality=self.personality, model=self.ollama_model
        )
...
def call_openai(
    prompt, model="gpt-4-turbo", base_url="https://api.openai.com/v1", temperature=0.7
):
    """Call OpenAI-compatible API endpoint"""
    api_key = os.getenv("NANOSAGE_API_KEY")
    if not api_key:
        raise ValueError(
            "NANOSAGE_API_KEY not found in environment variables or .env file"
        )
